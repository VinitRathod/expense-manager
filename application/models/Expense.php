...
class Expense extends CI_Model
{
    ...
    public function checkExpCode($expCode, $id)
    {
        if ($id != 0) {
            return $this->db->where(array('c_expcode' => $expCode, 'c_expid !=' => $id))->get('t_expcategories')->num_rows();
        } else {
            return $this->db->where('c_expcode', $expCode)->get('t_expcategories')->num_rows();
        }
    }
}
